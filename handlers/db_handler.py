...
class DBHandler:
    ...
    def upsert_page(self, document: dict) -> Optional[Dict[str, str]]:
        ...
        topics_to_score = {
            topic: score
            for topic, score in document['classes_bert'].items()
            if topic in ITOPICS and score > 0.5
        }
        ...
        for idx, (topic, score) in enumerate(sorted(topics_to_score.items(), key=lambda x: x[1], reverse=True)):
            if idx == 0 or score > self.SCORE_THRESHOLD:
                ...
                break
        ja_snippets: Dict[str, str] = reshape_snippets(document['snippets'])
        en_snippets: Dict[str, str] = reshape_snippets(document['snippets_en'])

        is_checked = 0
        is_useful = 1 if document['classes_bert']['is_useful'] > self.USEFUL_THRESHOLD else 0
        is_clear = document['classes']['is_clear']
        is_about_false_rumor = 1 if document['classes_bert']['is_about_false_rumor'] > self.RUMOR_THRESHOLD else 0
        ...
        if existing_page and orig['timestamp'] > existing_page['page']['orig']['timestamp']:
            ...
            document_['status'] = 'updated'
        elif not existing_page:
            self.collection.insert_one({'page': document_})
            document_['status'] = 'inserted'
        else:
            document_['status'] = 'ignored'
        return document_
    ...
    def search(self, ecountry: str, start: int, limit: int, lang: str, query: str):
        ...
        if ecountry:
            body = get_es_query(ECOUNTRY_ICOUNTRIES_MAP.get(ecountry, []))
    ...
    def get_pages(self, itopics: List[str], icountries: List[str], start: int, limit: int, lang: str) -> List[dict]:
        cur = self.collection.find(filter=self.get_filter(itopics, icountries), sort=self.get_sort(itopics))
